        ...
        jdbcTemplate.update("""
                INSERT INTO t_research_evidence
                    (evidence_id, run_id, kb_id, doc_id, document_name, document_version,
                     ...
                     source_metadata_hash, retrieved_by_task_id, truncated, read, source_extent)
                SELECT ?, id, ?, ?, ?, ?, ?::jsonb, ?, ?, ?, ?::jsonb, ?, ?, ?, ?, ?
                FROM t_research_run WHERE id = ? AND owner_user_id = ?
                ...
                e.truncated(), e.read(), e.sourceExtent().name(), e.runId(), ownerUserId);
