...
class ChunkRankingTest {
    ...
    @Test
    @DisplayName("出口有序，不依赖调用方传入已排序的候选")
    void sortsUnorderedBackendResult() {
        // 「出口有序」是本类兑现的契约而非入参前置条件：后端返回乱序（如 PG relaxed_order）时也必须被兜住
        List<RetrievedChunk> merged = ChunkRanking.sortedByScore(
    ...
    void sortsByScoreDescending() {
        List<RetrievedChunk> merged = ChunkRanking.sortedByScore(
                List.of(chunk("p1", 0.9F), chunk("p2", 0.3F), chunk("s1", 0.6F)));
    ...
    void nanScoreSinksToBottom() {
        ...
        List<RetrievedChunk> merged = ChunkRanking.sortedByScore(
                List.of(chunk("nan", Float.NaN), chunk("low", 0.1F), chunk("high", 0.9F)));
    ...
    void positiveInfinitySinksToBottom() {
        ...
        List<RetrievedChunk> merged = ChunkRanking.sortedByScore(
                List.of(chunk("inf", Float.POSITIVE_INFINITY), chunk("low", 0.1F), chunk("high", 0.9F)));
