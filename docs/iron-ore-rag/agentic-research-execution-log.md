...
2026-09-18，用户明确要求代码改进后暂停、不再测评以节省 5h 额度。S2 代码改进和必要回归已完成，实际应用复测已按要求停止并清理随机任务库；S3 32 题/96 项未启动，S4 未完成。当前恢复入口是[最新交接第 7 节](agentic-research-resume-2026-09-18.md#7-s2-代码改进交付与用户要求暂停)及[S2 暂停清单](../../eval/agentic-research/manifests/research-s2-pause-2026-09-18.json)；第 6 节和下段为暂停前状态。用户再次要求继续之前不自动测评。
...
## R3：有效阅读与受控检索对照（2026-09-18）

主 Agent 和 worker 在新检索后先读相关候选，服务端拒绝跳过阅读继续搜索，完全重复的查询/文档组合最多两次。提示显示直接阅读的文档和候选中未读文档，但不强制阅读所有干扰文档。邻接读取保持原有权限与快照边界，使用实际返回的新 ID。模型输入投影去掉存储/来源元数据，保留正文、文档身份、章节、截断和来源状态；完整快照及 SOURCE_READ 事件不变。目录、公式占位符和极短片段增加阅读提示，不把启发式提示当成事实错误判定。主/worker 提示升级为 v4/v3。

121 项相关后端检查通过（原 native 协议断言更新为新的 read_source 选择后，23 项受影响用例复跑通过），Python 30 项通过。12 原题×ABC 已按逐题轮换 ABC/BCA/CAB 开始真实复测；同 12 题 A 的 qwen3-rerank 对照等待基线结束。源码和运行类已冻结，后续修改不混入这些批次。重排和 thinking 为离线显式开关，当前默认仍关闭；结果与选择续记。


## R4：产物契约与运行问题分离（2026-09-18）
...
## S2：限制连续检索失败并按用户要求暂停（2026-09-18）

起始提交 `8bbff9a`；本阶段标题 `fix: bound repeated retrieval failures and pause career evaluation`。新增实际 HTTP 协议/连接复用记录；每个 Agent 连续两次检索失败后限制搜索和委派，读取旧候选仍允许；进入已有最多两次原生结束修复，额度不增加。无证据执行故障不能转换为 ask_user；纯执行错误允许空 findings/gaps/conflicts，通过 executionIssues 保存。成功检索重置当前错误，合法追问和引用边界保持程序检查。

217 项后端/33 项 Python 通过，随机测试库清理；最后空 gaps 契约修改后，30 项本地协议/连接/工具测试通过，不重复累计。真实客户端短检查 2/2 成功、已知 8 tokens，但不能认定外部服务稳定。两条 C 应用复测已启动，随后收到用户“改进完成后可以不用进行测评暂停任务，防止 5h 额度被用完”；只停止精确任务 JVM，JAVA_EXIT_143，随机库删除。plan-06 导出中止相关 FAILED，comparison 已有 worker/检索但未导出终态；没有可验收产物。embedding 捕获 6 请求、3 成功/3 unknown、已知 27 tokens；模型已导出 6 次 Max 主角色、input 28644/output 973，在途另一个任务模型台账不完整，不称为整批总量或整批 unknown 0。记录限制和负例全部保留。

S3 的 32 题/96 项未启动，S4 未完成。本轮代码已交付，业务集成/语义验收与后续工作依用户要求暂停，只有明确恢复才继续；不把暂停标为质量通过。见[S2 清单](../../eval/agentic-research/manifests/research-s2-pause-2026-09-18.json)、[最新交接第 7 节](agentic-research-resume-2026-09-18.md#7-s2-代码改进交付与用户要求暂停)，原始日志位于 `20260918_S2_validation` / `20260918_S2_client_probe_v1` / `20260918_S2_applications_v1`。
