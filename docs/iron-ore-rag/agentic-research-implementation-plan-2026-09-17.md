...
> **2026-09-18 起已被取代**：后续工作按[秋招冲刺计划（第 4 版）](career-sprint-plan-2026-09-18.md)执行，本文件 §8.7 未执行的 S3/S4 与 R5 接续作废；以下正文保留为历史，不再维护。
...
>
> 2026-09-18 秋招交付约定：用户要求先成批实现、在版本完成后集中验收，允许效果提升有限，不把每项改动的收益实验设为实施前置条件。按 [8.7](#87-秋招版本实施与集中验收2026-09-18)推进；本轮先接入角色模型配置，再处理阻塞运行的问题和集中验证。历史 R5 及其失败记录保留，新模型配置使用独立运行批次。
>
...
### 8.7 秋招版本实施与集中验收（2026-09-18）

用户已授权把本约定写入计划并开始实施。目标是尽快交付业务合理、能够演示和解释的求职版本；允许收益有限或部分任务落后，不要求所有公开数据集都超过基线。本节调整开发和验收顺序，优先于前文将每个改动都拆成多组收益对照的安排；真实调用留档、失败保留和必要回归继续遵循 8.5。

**固定主线：**普通问题使用已有 RAG；复杂资料问题使用检索、原文阅读、按缺口补查及有限委派，产出带来源的报告或计划草稿。借鉴成熟开源项目的研究机制，记录迁移理由、个人实现范围和实际取舍。P0—P8、R1—R4 已交付的主体能力直接复用。

**开发方式：**相关功能作为一组完成；期间只做编译、受影响的关键回归及少量真实兼容性检查。收益测评集中在稳定版本之后进行，不为每个提示或配置改动增加模型、数据集、消融组合。指标用于解释效果与选择默认配置；已有明确缺陷仍修复，不能把无依据事实当作正确产物。

| 阶段 | 本轮交付 | 验证和边界 |
| --- | --- | --- |
| S1 角色模型配置 | 主 Agent、worker、最终生成分别选择注册模型；初始部署为主/最终使用 `qwen3.7-max-2026-05-20`，worker 使用 `qwen3.7-flash-2026-07-15`；未配置角色时沿用旧 `research.model-id` | 三种角色继续共享调用额度、并发配额和取消机制；逐次记录实际模型与角色，评测冻结完整角色配置。先检查真实工具调用及结构化生成兼容性，不预先承诺质量或资源收益 |
| S2 运行阻塞与集成 | 针对 embedding 连续无响应做短批验证，修复妨碍研究流程完成的明确问题；贯通报告和计划 | 不扩大为无限网络根因调查；参数或端点变化保存到新配置。检索错误与原文缺口分开，未知参数继续保留 |
| S3 集中验收 | 稳定构建后统一执行固定对照、代表性复跑及比较/PLAN 原文核对；整理少量同一技术问题下的业务演示 | 不以每次改动先获胜作为启动条件。新角色模型与旧 Flash 结果分批报告；架构比较使用相同角色映射与最终生成设置。失败进入分母，开发回归与独立测试分别命名 |
| S4 求职交付 | 更新运行方式、演示、评测结论和可用于简历的实现/指标边界 | 简历可描述成熟机制迁移和完整能力，只有实测且口径明确的数字才作为提升结论；报告保留限制和负例 |

S1 仅新增角色选择和可复现配置，不增加新的 Agent 层级、自动业务执行、通用动态模型路由平台或在线服务。模型由现有注册表和提供方配置创建，复用已有凭证；无角色配置的历史运行路径保持一致。明确失败时返回错误，不静默换成另一个模型以美化结果。

S1—S2 的少量真实调用用于兼容性与流程检查，不作为正式效果结论。S3 继续保留约定的真实固定测评，不将实现优先解释为跳过必要评测。旧 R5 v2 及其 22 条记录不覆盖；修改源码后新建运行目录，禁止在原冻结批次混入新代码或角色模型。full 17517 任务继续作为执行能力，不列为秋招交付前置条件。

为满足时间约束，S3 首批使用已有固定开发集中的 32 题（QASPER/MuSiQue 各 16，ABC 共 96 项），按 `SHA256(20260918-career-v1:<questionId>)` 排序选取，不读取答案或历史输出；ID 固定在[秋招验收题单](../../eval/agentic-research/manifests/research-career-case-ids-2026-09-18.json)。这是旧开发回归的子集，不是 unseen holdout。先完成这一组同题比较、代表性复跑和应用原文核对，再整理简历；历史 R5 的完整 400×ABC 接续为后续扩大验证，不再作为首版秋招交付的前置条件。混合角色策略的结果不能单独归因为多 Agent 架构或 Max 模型。

**完成标准：**约定能力可以使用，主要路径无已知阻塞，必要回归通过，真实调用与固定测评如实留档，有可重复的业务演示，并能够解释借鉴机制、失败案例及适用边界。总体 F1、成本或并行收益不设预先承诺。

**当前执行状态与暂停约定：**S1 已实现并完成兼容性检查。S2 本轮补齐连续两次检索失败后的停止补查、有限原生结束、禁止无证据检索错误转为用户追问、纯执行错误可用空 gaps 结束，以及实际协议/连接复用记录。217 项后端及 33 项 Python 回归通过；最后结束契约调整再以 30 项本地协议测试通过确认。实际 embedding 客户端两次短检查均成功，但应用中仍有超时，不能宣布外部检索稳定。用户随后明确要求“改进完成后可以不用进行测评暂停任务，防止 5h 额度被用完”；已停止在途的两条应用复测并清理随机库，保存中止记录。S3 的 32 题/96 项没有启动，S3—S4 及业务语义验收暂停，用户再次要求继续后才执行。暂停不代表全部完成，也不补写质量数字。详见[S2 暂停清单](../../eval/agentic-research/manifests/research-s2-pause-2026-09-18.json)与最新交接，旧负结果保持不变。
...
| S1 角色模型配置 | 已实现并完成 SDK/供应商兼容性检查；Max/Flash/Max，未宣称质量提升 | `feat: select research models by role and preserve retrieval failures`；见本轮清单 |
| S2 稳定流程 | 代码改进已交付，集成验收暂停；217/33 回归及最后 30 项本地协议复核通过，外部检索仍有失败 | 两次实际客户端短检查成功；应用复测按用户要求中止，见 S2 暂停清单 |
| S3 集中验收 | 已暂停：32 题/96 ABC 项已固定并离线准备，未执行 | 用户要求节省 5h 额度，不启动质量测评 |
| S4 演示与简历 | 已暂停、未完成 | 现阶段可描述实现；当前版本的质量数字待实际验收后填写 |

后续可直接使用下面的实施指令：

> 用户于 2026-09-18 要求代码改进完成后暂停、不再测评，以节省 5h 额度。先阅读本计划 8.7、最新交接第 7 节及 S2 暂停清单，报告现状；只有用户再次要求继续时才接续真实调用、S3 集中验收与 S4。既有 Git/供应商授权仍有效，但不覆盖在暂停期间自行启动工作。新调用使用新的冻结目录，保留所有负例和中止记录；未经实测不得填写当前版本质量、多 Agent 收益数字。
