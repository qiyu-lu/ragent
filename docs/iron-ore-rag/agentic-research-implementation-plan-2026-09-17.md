...
> 制定日期：2026-09-17。状态：计划已制定，P0—P8 均未实施。
>
> 本文件是后续实施的主要交接入口。它记录本轮已经确定的产品方向、技术选择、删除范围、数据准备、阶段提交和验证方式。后续无需重新阅读完整聊天，也不要重新把方向改回送检助手。
>
> 本次只编写计划，没有删除代码、切换分支、提交 Git、下载数据或运行模型评测。文中的类、接口和配置若标为“拟新增”，均不是现有实现。
...
- 当前 HEAD：3577d6d，提交标题为 fix: support isolated RocketMQ store relocation to external disk。
- 已有未提交内容：docs/iron-ore-rag/README.md；未跟踪的 docs/assets/ 和 docs/iron-ore-rag/flow-notes/00-interview-summary.md。它们属于原有工作，不得顺手覆盖或打包提交。
