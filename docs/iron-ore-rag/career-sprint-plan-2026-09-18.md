...
| W6 | W1 的质量护栏：成本降了，答案有没有变差（2026-09-19 补入） | 配对答案 F1 差值及区间、可答题拒答数、同代码重复运行的噪声 | 1 天（另加约 6 小时后台运行） | `career-w6` |
| W7 | 容量基准：多实例排空积压（2026-09-19 补入） | 排空吞吐随实例数的变化、排队等待、重复领取数、轮询的数据库开销 | 0.5—1 天 | `career-w7` |
| S6 | 改动说明、简历条目 | — | 0.5 天 | `career-done` |

顺序固定为 S0 → W1 → W3 → W2 → W4 → W5 → W6 → W7 → S6（W6、W7 是验证而非新功能；W7 时间不够可砍，W6 不可砍——它守的是头牌数字）：W1 最小且数字最有冲击力；W3 产出的模拟上游是 W2 的测试基础。每个工作项结束即可更新一条简历，不必等全部完成。时间不够时从后往前砍（W5、W4），**S0 + W1 + W3 + W2 是底线**，约 6.5 天。
...
### W6 W1 的质量护栏：成本降了，答案有没有变差

**真实问题**：面试官必问“省了 70% 输入成本，代价是什么”。现有证据答不了：80 题复核（可答 45）F1 0.443→0.352，配对差 −0.09，区间 [−0.21, +0.02]；每臂只跑一次、未交错，不知道同一份代码跑两次本身差多少；15 道下降题里 5 道是“之前答对、之后拒答”，其余多为格式差异（`100` 对 `100th`）。

**样本从哪里来**：用已有的 MuSiQue dev 全量题，不从企业资料出题。

- `research_corpus_v1` 已导入 dev 全部 26,326 个段落（库 `rs_musique_dev_v1_full`），所以 `queries.jsonl` 的 4,834 题（可答 2,417）都能直接跑，不需要重新导入或向量化；评测器 `--profile full --case-ids <文件>` 已支持。
- 企业资料没有标准答案。自己出题、自己判分就是自造指标（§0 选题三问第 3 条），而且几十个块的小语料触发不了多轮检索和上下文压缩，测不到 W1 改动的部分。企业资料只用于演示，不进入这项统计。
- 样本量：现有配对差的标准差约 0.39（由 n=45 的区间半宽反推）。要把区间半宽压到约 0.05，需要约 240 道可答题。取 400 道新题（可答约 200）加已有 45 道，合计约 245。

**步骤**

1. **归因（零接口费，先做）**：新增 `eval/agentic-research/quality_diff.py`，读两个运行目录的逐题输出与 `traces.jsonl`，把每道下降题归为拒答 / 格式差异 / 答错，并对拒答题比较两臂的：是否取到 gold 证据、主 Agent 调用数、是否发生 `CONTEXT_COMPACTED`、结束时剩余预算。先在现有 `career_X1_mq80_before_C / after_C` 上跑，得到假设（候选：末尾提醒让模型更早收手；压缩存根丢了答案所在正文；纯噪声）。
2. **噪声基线**：当前 HEAD 在同 80 题上再跑一次 C 模式（约 25 分钟），与已有 `after_C`（`73deab5`）配对。差值接近 0 同时说明两件事：运行间噪声小、瘦身重构没有改变研究路径的行为。
3. **扩样**：`manifests/career-w6-case-ids-{1..4}.json`，每份 100 题。抽样规则沿用数据集工具的做法：按 SHA256(种子 `20260919`, 题目 ID) 排序，排除回归集 200 题（开发期见过），同一 `source_question_id` 只取先出现的一个版本；**不读答案与可答标签**。前：`career-v0-baseline`；后：当前 HEAD；C 模式、`configs/career-x1.json` 不变。按 前1 → 后1 → 后2 → 前2 → 前3 → 后3 → 后4 → 前4 交错，抵消上游随时间的漂移。扩展 `scripts/career-x1.sh` 支持 `--profile full` 与分块交错；用户用 `nohup` 后台跑，预计共约 6 小时，可 `--resume`。
4. **判定规则（跑之前写死）**：主指标是可答题的配对答案 F1 差值（bootstrap 95% 区间，种子与重采样次数沿用 80 题复核），新 400 题单独报告，再与旧 45 题合并报告；副指标是可答题拒答数、EM、证据 F1、缓存命中率（确认 −72% 在大样本上仍成立）。
   - 区间含 0 且 |均值| < 0.03：结论为“未见可测的质量变化”，W6 结束。
   - 区间不含 0：按第 1 步的归因修一次（限时半天，例如调整末尾提醒的措辞或位置、压缩存根保留答案句），只在同 400 题上重跑“后”臂，同时确认命中率没有回落。
   - 修后仍显著下降：不再追，如实写成取舍——成本 −x%，F1 −y，并写明下降集中在哪一类题。
5. 格式差异类的下降**不改评分口径**（EM / F1 保持数据集原定义），只在报告里按类别拆开列出。

**验收**：`manifests/career-w6-<日期>.json`（提交号、题目 ID 文件的 SHA、逐臂汇总、配对差与区间、噪声基线、下降题分类计数）；结论追加到 W1 改动说明的“限制”一节，不另写新文；状态文件的 W1 复核条目改写为最终结论。

**提交**：`feat(eval): classify per-question answer changes between two runs`；`test(research): widen the W1 answer-quality recheck to 400 unseen questions`；若有修复 `fix(research): …`；标签 `career-w6`。

### W7 容量基准：多实例排空积压

**真实问题**：后端面试官必问“能扛多少并发、瓶颈在哪、加机器有没有用”。W2 证明了任务不丢，但没有任何吞吐数字，也没有证明“队列满不再失败”在积压下成立。

**设计要点**

1. 复用 W3 的模拟上游（固定延迟，例如每次模型调用 700—900 ms，与 X2 一致）与 `x2_takeover.py` 的多 JVM 启动、随机运行库、不变量检查；新增 `eval/agentic-research/x6_capacity.py` 与 `scripts/career-x6.sh`。零接口费。
2. 负载：一次性提交 300 个脚本化任务到实例 1；实例数 1 / 2 / 3，每实例 `max-concurrent-runs` 固定（记录取值）；每格重复 3 次。租约与轮询用生产默认值（30 s / 5 s），另跑一组轮询 1 s 作对照。
3. 指标：排空吞吐（任务 / 分钟）与相对单实例的加速比；排队等待 P50 / P95（创建到首次领取）；提交阶段被拒绝的任务数（期望 0，任务留在库里等）；重复领取数（期望 0：每个任务 `attempt_count = 1`、恰好一个终态产物、事件序号连续）；领取语句的调用次数与平均耗时（`pg_stat_statements`，镜像未启用则在应用侧计时）；空闲时每实例的轮询 QPS。
4. 不做：真实上游下的压测（瓶颈是供应商限流，测的是别人的系统）、HTTP 入口的 QPS 压测。

**验收**：`manifests/career-x6-<日期>.json`；结论以“容量”一节追加到 W2 改动说明，写明瓶颈位置（预期：吞吐 ≈ 实例数 × 槽位 ÷ 单任务时长，数据库领取不是瓶颈；若不是线性，查明原因并如实写）。数字注明“基于模拟上游”。

**提交**：`test(research): backlog drain benchmark across instances`；标签 `career-w7`。
...
| X1b 质量护栏（W6） | MuSiQue dev 全量题中抽 400 道未见过的题（4 × 100，前后交错）；C 模式；`career-v0-baseline` 对当前 HEAD；另在旧 80 题上重复“后”臂一次作噪声基线 | 配对答案 F1 差值与区间、拒答数、下降题分类、命中率复现 |
| X6 容量（W7） | 模拟上游；300 个积压任务 × 1 / 2 / 3 实例 × 3 次重复 | 排空吞吐与加速比、排队等待、拒绝数、重复领取数、领取语句开销 |
...
**简历与面试的结构（2026-09-19 定）**：不平铺五条。主讲两条——W2 持久化执行（含 W7 的容量数字）、W1 提示缓存（含 W6 的质量结论）；副讲一条——W3 故障注入（同时是 W2 的测试基础）；W4、W5 各一句话，被问到再展开。面试卡以“方案取舍”为主线（为什么用数据库当队列而不用已有的 RocketMQ；为什么至少一次是安全的；为什么阈值压缩而不是滑动裁剪），并单列一张“这是 demo 吗”的应答卡：承认未上线，强调问题来自调用台账与事故数据、验证靠故障注入与不变量、主动说出已知限制。简历与口述都要分清上游开源项目自带的部分和自己写的部分（研究模块与 W1—W7）。
