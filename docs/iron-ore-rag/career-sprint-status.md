...
S6：按计划 §10 的结构写简历与面试卡——主讲 W2 持久化执行（含 W7 容量数字）、W1 提示缓存（含 W6 质量结论），副讲 W3 故障注入，W4、W5 各一句话；面试卡以方案取舍为主线，另加一张“这是 demo 吗”的应答卡。临时工作树 `../ragent-w6` 可 `git worktree remove`。
