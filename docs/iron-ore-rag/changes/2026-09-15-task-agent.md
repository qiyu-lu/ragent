...
| 后端：任务状态、工具、模型适配、业务事务与规程检索 | 完成 | 本次后端提交；31 项定向测试通过，其中 27 项在 PostgreSQL 上复跑通过 |
| 前端：办理页面、确认、补充资料与恢复入口 | 完成，待提交 | TypeScript、生产构建及兼容配置下的定向 lint 通过 |
| 回归验证、复现与交付 | 验证完成，提交整理中 | 数据库脚本验证完成；存量链路 39 项回归通过 |
...
上面的端口和口令仅为本次隔离容器的临时测试配置，不是应用配置。复跑须先准备自己的测试实例并替换地址；不能把这个地址当作现存服务。本次测试不连接、不迁移现有 `ragent` 业务数据库。Maven 生命周期包含 Spotless apply，本次只为新增 Java 文件补入项目许可证头，提交前检查了工作区范围。
