...
| 日期 | `2026-09-18` |
| 所属阶段 | 秋招冲刺 W5：增量重建 |
| 状态 | 已实施；回归通过；X5 在模拟上游和真实上游（SiliconFlow）下各跑一次 |
| 分支 | `feat/llm-backend-hardening` |
| Git 提交 | 实现与迁移 `80125b0`；PostgreSQL 集成测试 `7d43998`；X5 命令与脚本 `2355966`；标签 `career-w5`；真实上游结果随后以 `docs:` 提交补入 |
...
V1.2→V1.3 需要重嵌入的块占 2/79 = 2.5%，模拟 token 是全量重嵌入的 3.0%。每一步之后，块表和向量表里这份文档的行数都等于块数。模拟上游一共收到 7 次 embedding 请求，与各步“上游调用”之和一致。

**真实上游**（SiliconFlow `Qwen/Qwen3-Embedding-8B`、1536 维，2026-09-19 单次运行，代码同 `2355966`；运行目录 `local-data/agentic-research/runs/career_X5_real_v1/`）。token 是供应商响应里返回的 `usage.total_tokens`，5 步上游失败 0 次。第一次尝试因 API Key 错误（HTTP 401）在第一步就停止，没有写入任何步骤记录；换对 Key 后在新建的库上完整重跑。
...
| 同一文档升级到 V1.3 | 79 | 77 | **2** | 1 | **1,368** | 0.29 s |
| 回退到 V1.2 | 79 | 79 | 0 | **0** | 0 | 0.03 s |

- 块数与命中数和模拟上游完全一致，说明复用率只由文本决定。
- V1.2→V1.3 升级：上游 token 从全量的 45,722 降到 1,368（2.99%），省下 44,354；每一步的块表与向量表行数都等于块数。
- 耗时只看量级：两次全量向量化分别是 34.7 s 和 9.2 s，同样 79 块、3 次调用，相差将近 4 倍，是供应商延迟的波动，而且只跑了一次。能下的结论是：命中时不调用上游，向量化耗时从“秒到几十秒、随供应商波动”降到亚秒级（查缓存表的时间），不据此报加速倍数。
