...
问答：RetrievalScopeResolver.resolve
  有效库 ∩ readableCollections(当前用户) → 意图绑定 ∩ 交集 → 定向 / 全局 / 补充
  → 向量（SQL WHERE collection_name IN …）、ES（空范围直接返回空）、图谱（补充路只收补充范围）
...
| 通道 | 结论 |
| --- | --- |
| Redis | 只有意图树、提示词、术语映射三份全局配置缓存，以及停止标记和任务归属，没有检索结果或答案缓存；意图树里的库绑定在解析范围时会与可读库求交 |
...
| 意图树、Agent 配置、示例问题等写接口 | 仍只校验登录。这些是全局配置，不含资料正文；绑定不可读的库也检索不到它。未收紧 |
