...
当前接入任务归属检查、纯知识检索与块级/邻接证据快照读写。邻接快照通过 `origin_evidence_id` 关联同一运行内的原候选，部分唯一索引保留其首次展开；Java INSERT 的同运行条件避免关联其他运行的证据。任务调度、租约、取消、事件写入和模型调用在 P3/P4 实现。`event_sequence` 为后续原子序号分配预留，事件表主键不能代替并发分配器。
