...
cases=$repo/eval/agentic-research/manifests/career-cache-case-ids.json
...
python3 "$trees/after/eval/agentic-research/cache_report.py" \
    --run before-B="$runs/${stamp}_before_B" --run after-B="$runs/${stamp}_after_B" \
    --run before-C="$runs/${stamp}_before_C" --run after-C="$runs/${stamp}_after_C" \
